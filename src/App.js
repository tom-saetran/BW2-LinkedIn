...
class App extends React.Component {
    ...
    componentDidMount = async () => {
        this.setState({ allprofiles: await this.crud_user.get() })
        this.setState({ me: await this.crud_user.get("me") })
        this.setState({ withID: await this.crud_user.get("5fc4ae95b708c200175de88d") })
    }

    componentDidUpdate = async (_previousProps, _previousState) => {
        if (this.state.me._id !== undefined && this.state.exp === null) this.setState({ exp: await this.crud_user.get(this.state.me._id) })
    }
    ...
    crud = {
        user: this.crud_user,
        experience: this.crud_experience,
        posts: this.crud_posts
    }
    ...
    render() {
        return (
            <Router>
                ...
                <Switch>
                    <Route render={routeProps => <Profile {...routeProps} crud={this.crud_experience} exp={this.state.experience} me={this.state.me} all={this.state.allprofiles} />} exact path={["/profile/:id", "/profile"]} />
