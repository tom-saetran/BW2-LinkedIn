...
export default class NavBar extends Component {
    ...
    componentDidMount = async () => {
        if (!this.props.match.params) this.setState({ isSelf: true })
        this.setState({ resultFromID: await this.props.crud.get("6098fa41619e5d00151f8f7f") })
    }

    componentDidUpdate = async (_previousProps, _previousState) => {
        if (_previousState.isSelf !== this.state.isSelf) {
            // here we know who we are
        }
        //if (this.props.me._id !== undefined) this.setState({ resultFromID: await this.props.crud.get(this.props.me._id) })
    }
